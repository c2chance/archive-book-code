	...
	$(function() {
		...
		$(function() {
			$('#dataview').treegrid({
				...
				collapsible : true,
				fitColumns : true,
				url : 'MODELFindGrid.action?time=' + time,
				idField : 'ICODE',
				treeField : 'NAME',
				columns : [ [ {
					field : 'NAME',
				...
				toolbar : [ {
					...
					iconCls : 'icon-add',
					handler : function() {
						$("#iframemain").attr("src", "modeledit.jsp");
			            $("#wedit").window('open');
					}
				}, {
					id : 'btncut',
...
function dataEdit(icode){
	$("#iframemain").attr("src", "modeledit.jsp?icode="+icode);
    $("#wedit").window('open');
}
...
function dataDelete(icode){	
	$.messager.confirm('询问','确定要删除吗?',function(r){   
	    if (r){   
	    	$.ajax( {
	    		type : "post",
	    		url : "MODELDelete.action?maindatauuid="+icode,					
