(function($) {
	$(function() {
		...
		$('#tt2').tree(
				{
					url : 'PERMISSIONFindByUUID.action?maindatauuid='
							+ JUDGE.getURLParameter("icode")
				});
		...
		$("#btnSave").click(
				function() {
					...
					// 拼装数据
					var maindata = "{'ROLEICODE':'" + roleicode
							+ "','MODELS':'" + models + "'}";
					...
					$.ajax({
						...
						success : function(data) {
							$.messager.alert("提示消息", "保存成功!", "info",
									function() {
										window.parent.$('#dataview').datagrid(
												'reload');
										window.parent.$('#wedit').window(
												'close');
									});
						}
					});
