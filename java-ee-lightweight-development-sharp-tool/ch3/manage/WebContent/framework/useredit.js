(function($) {
	$(function() {
		...
		if (!JUDGE.isNull(icode)) {
			var url = "USERFindByUUID.action?maindatauuid=" + icode;
			$.ajax({
				type : "post",
				url : url,
				...
				success : function(data) {
					$("#maindata")
							.fromJsonString(JSON.stringify(data.maindata));
				}
			});
		}
						...
						success : function(data) {
							if (data.returncount > 0) {
								...
								if (data.savetype == "insert") {
									$.messager.alert("提示消息", "新增保存成功!", "info",
											function() {
												window.parent.$('#dataview')
														.treegrid('reload');
												window.parent.$('#wedit')
														.window('close');
											});
								} else {
									$.messager.alert("提示消息", "编辑保存成功!", "info",
											function() {
												window.parent.$('#dataview')
														.treegrid('reload');
												window.parent.$('#wedit')
